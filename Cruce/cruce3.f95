...
real, dimension(:), allocatable :: xzmax, xzmin, yzmax, yzmin, zedx, zedy, pdx, pdy, pcx, pcy
...
real, dimension(:), allocatable :: tin, tout
logical, dimension(:), allocatable :: in, yapaso
...
allocate(zedx(4), zedy(4), pdx(4), pdy(4), pcx(4), pcy(4))
...
allocate(tin(N), tout(N), in(N), yapaso(N))
...
frot0 = 0.0
allocate(fov(2))
open(unit=0, file="ocruce2.dat", status="old", action="read")
nlineas = 0
do
...
open (unit=1, file="crot02d.dat", status="replace", action="write")
open (unit=2, file="cruce2b.dat", status="replace", action="write")
open (unit=3, file="cruce3.dat", status="replace", action="write")
open (unit=4, file="cruce4.dat", status="replace", action="write")
...
do h = 1, topeh
	...
	do i = 1, N
		...
		if ((iz == dest(i))) then
			...
			ud = (/pcx(dest(i))-x(i), pcy(dest(i))-y(i)/)
			edx(i) = (ud(1)/norm(ud)) + taupp * vx(i)/v0(i)
			edy(i) = (ud(2)/norm(ud)) + taupp * vy(i)/v0(i)		
		else if (iz /= 0) then
			ud = (/pdx(iz)-x(i), pdy(iz)-y(i)/)
			edx(i) = (ud(1)/norm(ud)) + taupp * vx(i)/v0(i)
			edy(i) = (ud(2)/norm(ud)) + taupp * vy(i)/v0(i)
		end if
		ud = (/edx(i), edy(i)/)
	...
	do i = 1, N
		...
		if (.not. in(i)) then
			...
			if (iz == dest(i)) then
				if (.not. yapaso(i)) then
					tout(i) = h*tpaso
					write (unit=4, fmt=*) tout(i)-tin(i)
					yapaso(i) = .true.
				end if
			end if
		end if
