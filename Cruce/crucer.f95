...
real, dimension(:), allocatable :: xz0max, xz0min, yz0max, yz0min, xcmax, xcmin, ycmin, ycmax
real, dimension(2) :: ud
...
allocate(xcmin(4), xcmax(4), ycmin(4), ycmax(4))
...
pdx = (/4.0, 0.0, -4.0, 0.0/)
pdy = (/0.0, 4.0, 0.0, -4.0/)
xcmin = (/0.0, 0.0, -4.0, -4.0/)
xcmax = (/4.0, 4.0, 0.0, 0.0/)
ycmin = (/-4.0, 0.0, 0.0, -4.0/)
ycmax = (/0.0, 4.0, 4.0, 0.0/)
...
do h = 1, topeh
	...
	do i = 1, N
		...
		if ((iz == dest(i))) then
			...
			call dz(x(i), y(i), xcmin, xcmax, ycmin, ycmax, dest(i), izc)
			if (izc == dest(i)) then
				edx(i) = zedx(dest(i)) + taupp * vx(i)/v0(i)
				edy(i) = zedy(dest(i)) + taupp * vy(i)/v0(i)
			else if (izc /= dest(i)) then
				edx(i) = -(x(i)/abs(x(i)) + y(i)/abs(y(i)))/2.0
				edy(i) = (x(i)/abs(x(i)) - y(i)/abs(y(i)))/2.0
			end if
		else if (iz /= 0) then
